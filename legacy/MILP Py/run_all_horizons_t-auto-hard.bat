...
echo Horizon,Start Time,End Time,Duration > timing_results_forced.csv
...
for %%h in (%horizons%) do (
    ...
    python "C:\Users\peram\Documents\test\MILP Py\t-auto-hard.py" --horizon %%h --start_gw !start_gw! --max_gw !max_gw!
    
    set end_time=!time!
    echo End time: !end_time!
    
    REM Calculate and display duration
    echo Horizon %%h complete! Results saved to Squad Selection t-auto-hard%%h.csv
...
echo Timing results saved to timing_results_t-auto.csv
echo =======================================================
pause